...
// GHL Config from .env (Subaccount 1)
const GHL_TOKEN = process.env.GHL_ACCESS_TOKEN;
const GHL_LOCATION_ID = process.env.GHL_LOCATION_ID;
const CALENDAR_ID = "tTy239HES3GhEtfiJAXq";
...
async function getGHLFreeSlots() {
    try {
        ...
        const response = await axios.get(`https://services.leadconnectorhq.com/calendars/free-slots`, {
            params: { calendarId: CALENDAR_ID, startDate: startTime, endDate: endTime },
        ...
        for (const day of days) {
            if (slotsObj[day].slots && slotsObj[day].slots.length > 0) {
                ...
                const times = slotsObj[day].slots.slice(0, 3).map(s => new Date(s).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }));
        ...
        console.error('Error fetching GHL slots:', err.message);
...
async function bookGHLAppointment(name, email, phone, startTime) {
    try {
        await axios.post(`https://services.leadconnectorhq.com/calendars/appointments`, {
            calendarId: CALENDAR_ID, locationId: GHL_LOCATION_ID, startTime: startTime,
            title: `AI Booking: ${name}`, email: email, phone: phone,
            firstName: name.split(' ')[0], lastName: name.split(' ').slice(1).join(' ') || 'Caller'
        }, {
            headers: { 'Authorization': `Bearer ${GHL_TOKEN}`, 'Version': '2021-04-15', 'Content-Type': 'application/json' }
        });
        return "Confirmed! I've placed that on my calendar for you. You'll get a confirmation text and email shortly.";
    } catch (err) {
        console.error('Error booking GHL appointment:', err.message);
        return "I've noted that time! There was a small lag, but our team will reach out to confirm.";
    }
}
...
app.post('/webhook/vapi', async (req, res) => {
    try {
        ...
        if (message.type === 'tool-calls') {
            ...
            let result = "";
            if (functionName === 'get_available_slots') result = await getGHLFreeSlots();
            else if (functionName === 'book_appointment') result = await bookGHLAppointment(args.name, args.email, args.phone, args.time);

            return res.status(200).send({ results: [{ toolCallId: toolCall.id, result: result }] });
        }
